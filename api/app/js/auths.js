...
function renderCityPicker(displayName) {
  // BUG FIX: displayName is the person's Google/Apple account name — third-
  // party data we don't control. It used to go straight into innerHTML
  // unescaped (a crafted display name could run script during login). esc()
  // (from utils.js) HTML-escapes it before it's templated in.
  document.getElementById("auth-method-form").innerHTML = `
    <div class="auth-form-slide auth-city-picker">
      <p class="auth-welcome-msg">👋 Welcome${displayName ? ", " + esc(displayName) : ""}!</p>
...
// Polls until the GSI script (loaded async/defer from accounts.google.com)
// has actually finished setting up window.google.accounts.id.
function ensureGSIReady(timeout = 8000) {
  return new Promise((resolve, reject) => {
    const start = Date.now();
    (function check() {
      if (window.google && window.google.accounts && window.google.accounts.id) return resolve();
      if (Date.now() - start > timeout) return reject(new Error("Google Sign-In script failed to load"));
      setTimeout(check, 80);
    })();
  });
}
...
async function startGoogleLogin() {
  if (!selectedRole) { showToast("Please select Customer or Driver first", "error"); return; }

  const btn = document.getElementById("method-btn-google");
  let mount = document.getElementById("google-btn-mount");
  if (!mount) {
    mount = document.createElement("div");
    mount.id = "google-btn-mount";
  ...
  try {
    ...
    // Most common real-world cause: the Google script is blocked (ad
    // blocker / offline) or never loaded. Surface it instead of failing
    // silently like the old prompt()-only flow did.
    showToast("Google Sign-In failed to load — check your connection", "error");
    console.error(err);
    return;
  }
  ...
  if (!gsiInitialized) {
    window.google.accounts.id.initialize({
      ...
      use_fedcm_for_prompt: true, // required by current Chrome/FedCM rules
    });
    gsiInitialized = true;
  }

  // BUG FIX: the old code only called google.accounts.id.prompt() (One Tap)
  // and, if Google reported the prompt as "not displayed" or "skipped",
  // rendered a fallback button into a detached <div> that was never
  // attached to the page (so it could never be seen, let alone clicked).
  // One Tap is suppressed silently in a lot of real situations (blocked
  // third-party cookies, FedCM opt-out, the user dismissed it before), so
  // relying on it as the primary path made the whole "Continue with Google"
  // button look broken.
  //
  // Now we always render Google's actual Sign-In button right here — it's
  // a real Google-controlled element, so clicking it always works, unlike
  // trying to trigger Google's OAuth flow from our own custom button.
  const width = Math.max(200, Math.min(Math.round(btn?.offsetWidth || 320), 400));
  mount.innerHTML = "";
  window.google.accounts.id.renderButton(mount, {
    theme: document.documentElement.classList.contains("light-theme") ? "outline" : "filled_black",
    size:  "large",
    shape: "pill",
    text:  "continue_with",
    width,
  ...
  // One Tap is still attempted as a bonus on top — if it shows, great; if
  // not, the rendered button above is already there as the reliable path.
  window.google.accounts.id.prompt(notification => {
    if (notification.isNotDisplayed?.()) {
      console.info("Google One Tap not shown:", notification.getNotDisplayedReason?.());
    }
  });
}
...
async function handleGoogleCallback(response) {
  if (!response || !response.credential) {
    showToast("Google sign-in failed — no credential returned", "error");
    return;
  }
  let data;
  try {
    data = await api("/auth.php?action=google_login", "POST", {
      id_token: response.credential,
      role: selectedRole,
      remember_me: true,
    });
  } catch (err) {
    console.error("google_login request failed:", err);
  ...
  // Credentials verified by the server — proceed straight to the dashboard
  // with the session (JWT/token) it returned.
  handleAuthSuccess(data);
}
...
// BUG FIX: handleAuthSuccess() used to call showPage()/loadXDashboard()
// unconditionally, assuming the dashboard markup lives on this same page
// (true for the original single-file index.php SPA). On a standalone landing
// page — which has no #page-customer-dashboard / #page-driver-dashboard
// elements — that call would throw and silently break the login flow. This
// checks which structure is present and falls back to a real redirect that
// carries the session (the token/user are already in localStorage/
// sessionStorage by the time this runs, so dashboard.html picks them up
// immediately on load).
function goToDashboard(role, status) {
  if (role === "customer") {
    if (document.getElementById("page-customer-dashboard")) {
      showPage("page-customer-dashboard");
      loadCustomerDashboard();
    } else {
      window.location.href = "dashboard.html?role=customer";
    }
  } else if (role === "driver-review") {
    if (document.getElementById("page-under-review")) {
      showPage("page-under-review");
      renderUnderReview(status || "pending");
    } else {
      window.location.href = "under-review.html?status=" + encodeURIComponent(status || "pending");
    }
  } else if (role === "driver") {
    if (document.getElementById("page-driver-dashboard")) {
      showPage("page-driver-dashboard");
      loadDriverDashboard();
    } else {
      window.location.href = "dashboard.html?role=driver";
    }
  }
}
...
function handleAuthSuccess(data) {
  ...
  // Guard: only proceed to the dashboard if the server actually gave us a
  // session to carry there. ("if credentials are true" → we only redirect
  // once we genuinely have a token + user back from the server.)
  if (!data || !data.token || !data.user) {
    showToast("Login failed — no session returned by server", "error");
    console.error("handleAuthSuccess called without a valid session:", data);
    return;
  }
  ...
  if (currentUser.role === "customer") {
    ...
    if (data.driver_status === "active") {
      ...
      goToDashboard("driver-review", data.driver_status || "pending");
    }
  }
}
...
function renderUnderReview(status) {
  ...
  const target = document.getElementById("under-review-content");
  if (!target) return;
  target.innerHTML = `
